...
export const SlideRenderer: React.FC<SlideRendererProps> = ({ slide }) => {
  ...
  const Placeholder = ({ index, className = "", fit = "cover" }: { index: number; className?: string; fit?: "cover" | "contain" }) => {
    ...
    return (
      <div 
        ...
        className={`relative group cursor-pointer overflow-hidden bg-zinc-900 flex items-center justify-center border border-zinc-800/50 ${className}`}
      >
        <img 
          src={src} 
          alt={slide.title} 
  ...
  switch (slide.type) {
        ...
        <div className="flex h-full w-full bg-black overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2 h-full w-full">
            ...
            <div className={`${isImageLeft ? 'order-1 md:order-1' : 'order-1 md:order-2'}`}>
              <Placeholder index={0} fit="cover" className="h-full" />
            </div>
          </div>
        </div>
      );
    ...
    case SlideType.FEATURE:
      return (
        <div className="relative h-full w-full bg-zinc-950 overflow-hidden flex flex-col lg:flex-row">
           ...
           <div className="w-full lg:w-2/5 p-8 lg:p-24 flex flex-col justify-center relative z-10 lg:-ml-32">
              <div className="bg-black/60 backdrop-blur-md p-10 lg:p-16 border-l-8 border-[#ff4d00] shadow-2xl">
      ...
      return (
        <div className="flex flex-col h-full bg-zinc-950 overflow-hidden">
          <div className="flex-1 p-12 lg:p-20 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {slide.list?.map((item, i) => {
                const [label, val] = item.split(': ');
                return (
                  <div key={i} className="p-8 bg-black/40 border border-zinc-900 transition-all hover:bg-zinc-900/50">
          ...
          <div className="h-1/3 grid grid-cols-3 gap-0 bg-black border-t-2 border-[#ff4d00]/20">
            <Placeholder index={0} fit="contain" className="bg-white border-r border-zinc-900" />
            <Placeholder index={1} fit="contain" className="bg-white border-r border-zinc-900" />
